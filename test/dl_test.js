...
describe("dl.php", function () {
	...
	describe("release channel", function () {
		it("should offer 5.0 for Mac by default", async function () {
			...
			assert.match(result, /standalone\/5.0\/Zotero-5.0.dmg$/);
		});
		...
		it("should offer 4.0 for Mac with from=4.0", async function () {
			...
			assert.match(result, /standalone\/4.0.29.15\/Zotero-4.0.29.15.dmg/);
		});
		...
		it("should offer 5.0 for Windows by default", async function () {
			...
			assert.match(result, /standalone\/5.0\/Zotero-5.0_setup.exe$/);
		});
		...
		it("should offer 4.0 for Windows with from=4.0", async function () {
			...
			assert.match(result, /standalone\/4.0.29.17\/Zotero-4.0.29.17_setup.exe/);
		});
		...
		it("should offer 5.0 for Linux x86_64 by default", async function () {
			...
			assert.match(result, /standalone\/5.0\/Zotero-5.0_linux-x86_64.tar.bz2$/);
		});
		...
		it("should offer 4.0 for Linux x86_64 with from=4.0", async function () {
			...
			assert.match(result, /standalone\/4.0.29.10\/Zotero-4.0.29.10_linux-x86_64.tar.bz2/);
		});
	});
	...
	describe("beta channel", function () {
		it("should offer 5.0 Beta for Windows", async function () {
			...
			assert.ok(result.match(/standalone\/beta\/5.0-beta.+Zotero-5.0-beta/));
		});
	});
});
